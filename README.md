...
로그인 뒤 화면 상단의 **라이트/다크** 버튼으로 색상 모드를 바꿀 수
있습니다. 글을 선택하면 주소에 해당 글 번호가 들어가므로 그 주소를 다시
열어도 같은 글이 선택됩니다. 최신 RSS 50개보다 오래된 글도 유효한 글
번호가 있는 주소라면 네이버에서 본문을 직접 불러옵니다.

새 글 알림을 실행하는 GitHub 저장소에는 아래 Actions secrets를 설정합니다.

- `TELEGRAM_BOT_TOKEN`: 텔레그램 봇 토큰
- `TELEGRAM_CHAT_ID`: 알림을 받을 채팅 ID
- `STREAMLIT_APP_URL`: 배포된 리더 주소(예: `https://blogtts-sh.streamlit.app`)

알림의 **오디오 리더에서 듣기** 링크에는 새 글 번호가 자동으로 붙으며,
로그인이 필요한 경우에도 로그인 후 요청한 글이 그대로 열립니다. 텔레그램
인증값은 코드의 기본값으로 두지 않고 Actions secrets에서만 읽습니다.
